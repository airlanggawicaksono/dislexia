...
async def test_create_admin_returns_username_and_temp_password():
    ...
    # In production admin_id is generated by the DB on flush; the mocked
    # refresh has to stand in for that.
...
# ─── regenerate_user_code ─────────────────────────────────────────────────────
def _make_user(account_number: str = "123456") -> User:
    user = User(account_number=account_number, display_name="quiet-otter")
    user.user_id = uuid4()
    user.is_active = True
...
@pytest.mark.asyncio
async def test_regenerate_user_code_overwrites_and_keeps_identity():
    user = _make_user("123456")
    db = AsyncMock()
    # 1st execute: user lookup; 2nd: uniqueness check (no collision).
    db.execute.side_effect = [_scalar_returning(user), _scalar_returning(None)]
    service = AdminService(db)

    res = await service.regenerate_user_code(user.user_id)

    assert res.user_id == user.user_id
    assert res.display_name == "quiet-otter"
    assert res.account_number == user.account_number  # overwritten in place
    assert len(res.account_number) == 6 and res.account_number.isdigit()
    assert db.commit.called


@pytest.mark.asyncio
async def test_regenerate_user_code_retries_on_collision():
    user = _make_user("123456")
    other = _make_user("654321")
    db = AsyncMock()
    # user lookup, then collision on 1st candidate, free on 2nd.
    db.execute.side_effect = [
        _scalar_returning(user),
        _scalar_returning(other),
        _scalar_returning(None),
    ]
    service = AdminService(db)

    res = await service.regenerate_user_code(user.user_id)

    assert len(res.account_number) == 6
    assert db.commit.called


@pytest.mark.asyncio
async def test_regenerate_user_code_unknown_user_raises_404():
    db = _mock_db_returning(None)
    service = AdminService(db)

    with pytest.raises(NotFoundError):
        await service.regenerate_user_code(uuid4())
