"""Tests for app/routers/features/professionalize.py"""
...
_BASE = "/api/v1/me/professionalize"


def test_process_stream_endpoint_registered():
    """Migration guard: streaming-only — /process-stream must stay
    registered and the sync /process must NOT come back.
    DB-free: checks route registration only."""
    from app.main import app
    paths = {getattr(r, "path", "") for r in app.routes}
    assert f"{_BASE}/process-stream" in paths
    assert f"{_BASE}/process" not in paths




def test_process_stream_requires_auth(client: TestClient):
    res = client.post(f"{_BASE}/process-stream", json={"text": "hi"})
...
def test_endpoints_reject_bad_auth(client: TestClient, headers: dict, case: str):
    res = client.post(f"{_BASE}/process-stream", json={"text": "hi"}, headers=headers)
