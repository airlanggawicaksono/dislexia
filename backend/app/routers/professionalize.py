...
router = APIRouter(prefix="/api/v1/{access_hash}/professionalize", tags=["Professionalize"])


@router.post("/process", response_model=FeatureResponseDTO)
async def professionalize_text(request: FeatureRequestDTO, access_hash: str, db: AsyncSession = Depends(get_db)):
    """
...
async def get_professionalize_history(access_hash: str, db: AsyncSession = Depends(get_db)):
    """
