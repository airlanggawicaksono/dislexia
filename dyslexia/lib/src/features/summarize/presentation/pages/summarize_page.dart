...
class _SummarizeBodyState extends State<_SummarizeBody> {
  ...
  void _submit() {
    ...
    if (text.isNotEmpty) context.read<SummarizeBloc>().add(SummarizeTextEvent(text));
  }
      ...
      builder: (context, ds) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: narrow
                ? [
                    _FeatureBarAction(icon: Icons.auto_awesome, label: 'Summarize', color: Colors.white, backgroundColor: const Color(0xFF3D5A99), onTap: _submit),
                      ...
                      child: BlocBuilder<SummarizeBloc, SummarizeState>(
                        builder: (context, state) {
                          return switch (state) {
                            ...
                            SummarizeErrorState(:final message) => Center(child: Text(message, style: const TextStyle(color: Colors.red))),
                            _ => const SizedBox(),
                          };
                        },
                      ),
